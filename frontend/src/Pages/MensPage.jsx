...
export default function Mens() {
  ...
  // Extract subcategory from URL (e.g., /mens/gold-rings -> "gold-rings")
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Load products filtered by Mens collection
        const list = await listBackendProducts({ collection: "Mens" });
        setProducts(list);

        // Load categories for Mens collection
      ...
      for (const cat of categories) {
        if (cat.subcategories && cat.subcategories.length > 0) {
          const matchingSub = cat.subcategories.find(sub => {
            ...
            // Handle special cases like "gold-rings" matching "Gold Rings"
            const subWords = subNormalized.split(' ');
            const urlWords = urlNormalized.split(' ');
            if (subWords.every(word => urlWords.some(uw => uw.includes(word) || word.includes(uw)))) return true;
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const isMens = productCollection === "mens";
    ...
    if (urlSubcategory) {
      ...
      if (productSub) {
        ...
        // Handle hyphenated matches
        const productSubNoSpaces = productSubNormalized.replace(/\s+/g, '-');
        if (productSubNoSpaces === urlSubcategory.toLowerCase()) return true;
      }
  ...
  const handleSubcategoryClick = (subcategory) => {
    ...
    const subcategorySlug = (subcategory || "")
      ...
      .replace(/\s+/g, '-')
      .replace(/[^a-z0-9-]/g, '')
      .replace(/\//g, '-'); // Replace / with - for "Gold Kadas/Bracelet" -> "gold-kadas-bracelet"
    
    navigate(`/mens/${subcategorySlug}`, { replace: true });
  };
